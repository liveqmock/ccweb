...
public class VGProductEntryBean implements Serializable {
    ...
    public List<ProductEntry> getActiveProductByIsdn(String isdn) {
        TypedQuery<ProductEntry> q = em.createQuery("SELECT sp.productEntry FROM SubscriberProduct sp WHERE sp.isdn = :isdn AND sp.supplierstatus != 3", ProductEntry.class);
